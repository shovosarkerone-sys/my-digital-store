    ...
    async function fetchSellerData() {
      ...
      try {
        ...
        if (sellerData) {
          ...
          // ডিফল্ট ভেরিফায়েড মার্চেন্ট অবজেক্ট
          setSeller({
            id: sellerId,
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white selection:bg-sky-500 selection:text-white transition-colors duration-200">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              ...
              className="text-xs text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-3 py-1.5 rounded-xl transition cursor-pointer font-medium"
            >
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm dark:shadow-2xl relative overflow-hidden transition-colors">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
            <div className="flex items-start gap-4 sm:gap-5">
              {/* Rank Avatar */}
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex items-center justify-center text-3xl sm:text-4xl shadow-inner shrink-0">
                {level.icon}
              </div>
              ...
              <div className="space-y-1.5 min-w-0">
                <div className="flex items-center gap-2.5 flex-wrap">
                  ...
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[11px] font-bold text-emerald-600 dark:text-emerald-400">
                ...
                <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 flex-wrap">
                  ...
                  <span>•</span>
                  <span>Region: <strong className="text-slate-700 dark:text-slate-200">{seller?.country || "Global"}</strong></span>
                  <span>•</span>
                  <span className="text-emerald-600 dark:text-emerald-400 font-bold">100% Positive Feedback</span>
                </div>

                <p className="text-xs text-slate-500 dark:text-slate-400 pt-1 font-mono">
                  Member since: <span className="text-slate-700 dark:text-slate-300">{seller?.created_at?.split("T")[0] || "2024-01-01"}</span>{" "}
                  <span className="text-sky-600 dark:text-sky-400">({getDuration(seller?.created_at)})</span>
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row md:flex-col items-stretch sm:items-center md:items-end gap-3 shrink-0">
              <button
                ...
                className="px-6 py-3 bg-sky-500 hover:bg-sky-600 text-white font-bold text-xs rounded-2xl shadow-md shadow-sky-500/20 transition transform active:scale-95 cursor-pointer flex items-center justify-center gap-2"
              >
                <span className="text-base">💬</span>
                <span>Send Private Message</span>
              </button>

              <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-xl text-[10px] font-semibold text-amber-700 dark:text-amber-300">
                <span>🛡️</span> 36-Hour Buyer Protection Standard
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-6 border-t border-slate-200 dark:border-slate-800/80">
            <div className="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-2xl border border-slate-200 dark:border-slate-800 text-center">
              <span className="text-[10px] text-slate-400 dark:text-slate-500 block uppercase font-bold">Listed Products</span>
              <strong className="text-base font-black text-slate-900 dark:text-white">{products.length}</strong>
            </div>
            <div className="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-2xl border border-slate-200 dark:border-slate-800 text-center">
              <span className="text-[10px] text-slate-400 dark:text-slate-500 block uppercase font-bold">Total Orders Sold</span>
              <strong className="text-base font-black text-sky-600 dark:text-sky-400">{totalSold}</strong>
            </div>
            <div className="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-2xl border border-slate-200 dark:border-slate-800 text-center">
              <span className="text-[10px] text-slate-400 dark:text-slate-500 block uppercase font-bold">Dispute Rate</span>
              <strong className="text-base font-black text-emerald-600 dark:text-emerald-400">0.0%</strong>
            </div>
            <div className="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-2xl border border-slate-200 dark:border-slate-800 text-center">
              <span className="text-[10px] text-slate-400 dark:text-slate-500 block uppercase font-bold">Avg. Response Time</span>
              <strong className="text-base font-black text-slate-900 dark:text-white">Under 15m</strong>
            </div>
          </div>
        </div>
        ...
        {activeTab === "products" && (
          <section className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <h2 className="text-base font-bold text-slate-900 dark:text-white">Items in this Merchant's Inventory</h2>
              ...
              {products.length > 2 && (
                <div className="relative w-full sm:w-64">
                  <input
                    ...
                    className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 focus:border-sky-500 rounded-xl px-3.5 py-1.5 pl-8 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none shadow-xs"
                  />
                  <span className="absolute left-2.5 top-2 text-slate-400 dark:text-slate-500 text-xs">🔍</span>
                </div>
              )}
            </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                        <div className="w-14 h-14 sm:w-16 sm:h-16 bg-slate-100 dark:bg-slate-800 rounded-xl overflow-hidden shrink-0 relative flex items-center justify-center border border-slate-200 dark:border-slate-700/60">
                          {product.image_url ? (
                            <img
                        ...
                        <div className="min-w-0 flex-1 space-y-1">
                          ...
                          <h3 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white truncate group-hover:text-sky-600 dark:group-hover:text-sky-300 transition">
                            {product.title}
                          </h3>
                          ...
                          <div className="flex flex-wrap items-center gap-1.5 text-[10px]">
                            {product.delivery_type === "manual" ? (
                              ...
                              <span className="bg-sky-500/10 text-sky-600 dark:text-sky-400 border border-sky-500/20 px-2 py-0.5 rounded font-bold">
                                ⚡ Auto Delivery
                              </span>
                            )}

                            {product.delivery_type !== "manual" && (
                              <span className="bg-slate-100 dark:bg-slate-950 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 font-medium">
                                Stock: <strong className={stock > 0 ? "text-emerald-600 dark:text-emerald-400 font-bold" : "text-rose-600 dark:text-rose-400 font-bold"}>{stock}</strong>
                              </span>
                            )}
                        ...
                        <div className="text-right">
                          ...
                          {hasDiscount ? (
                            <div className="flex items-baseline gap-1.5 justify-end">
                              <span className="text-[11px] line-through text-slate-400 dark:text-slate-500 font-mono">
                                ${product.price}
                              </span>
                              <span className="text-xs sm:text-base font-black text-emerald-600 dark:text-emerald-400 leading-tight font-mono">
                                ${product.discount_price}
                              </span>
                            </div>
                          ) : (
                            <span className="text-xs sm:text-base font-black text-sky-600 dark:text-sky-400 leading-tight font-mono">
                              ${product.price}
                            </span>
                          )}
                        </div>
                        <span className="bg-sky-500 group-hover:bg-sky-600 text-white text-[10px] sm:text-xs font-bold px-3 py-1.5 rounded-xl transition whitespace-nowrap shadow-md shadow-sky-500/20">
                          Buy Now →
                        </span>
                      </div>
                    </Link>
        ...
        {activeTab === "feedbacks" && (
          <section className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 space-y-4 shadow-sm dark:shadow-xl transition-colors">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              ...
              <span className="text-xs px-3 py-1 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 rounded-full font-bold">
            ...
            <div className="space-y-3">
              <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <strong className="text-xs text-slate-900 dark:text-white">Instant code delivery, perfectly working!</strong>
                  </div>
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">Verified Purchase</span>
                </div>
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-emerald-500 dark:text-emerald-400 font-bold">★★★★★</span>
                    <strong className="text-xs text-slate-900 dark:text-white">Fast communication and trusted seller</strong>
                  </div>
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">Verified Purchase</span>
                </div>
                <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
        ...
        {activeTab === "about" && (
          <section className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 space-y-4 shadow-sm dark:shadow-xl transition-colors">
            <div className="border-b border-slate-200 dark:border-slate-800 pb-3">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">
                Store Security & Buyer Protection
              </h3>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-2">
                ...
                <h4 className="font-bold text-slate-900 dark:text-white">36-Hour Buyer Protection Hold</h4>
                <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
                  Payment for orders from this merchant is held safely for 36 hours. If an activation key is invalid, the buyer is covered under our money-back guarantee.
                </p>
              </div>

              <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-2">
                <span className="text-lg">⚡</span>
                <h4 className="font-bold text-slate-900 dark:text-white">Instant Dispute Resolution</h4>
                <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
